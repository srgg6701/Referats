...
if($fl=='filter')
{
  ...
  if($problems=='on'){$S_F_MESS=$S_F_MESS.'1';}else{$S_F_MESS=$S_F_MESS.'0';}
}
//настройка фильтра
if($status==-1)
...
if($wstr[4]=='1'){$where=$where." AND (ri_mess.subj LIKE '%Проблемный заказ%')";}
...
<table width="100%" border="0" cellpadding="2" cellspacing="1" bgcolor="#CCCCCC">
  <tr bgcolor="#EEEEEE">
    <td colspan="10"><table width="85%" cellpadding="0"  cellspacing="0">
      <tr>
...
Исходящие</td>
    ...
    Клиенты</td>
	<td align="center" id="problems" nowrap><input type="checkbox" name="problems" class="checkboxFilter" id="problems" value="on" <?php if($wstr[4]=='1'){echo('checked');}?>> Проблемные</td>
          </tr>
        </table></td>
...
<!--    <td align="center" nowrap bgcolor="#CCFFCC"><a href="#"><b>ID</b></a></td>-->
    ...
    <td nowrap bgcolor="#CCFFCC"><a href="#"><strong>Имя</strong></a></td>
...
for($i=0;$i<$ms_n;$i++)
{
  ...
  if($mzakaz+1-1>0)
  {
	...
	if($zk_n>0)
	{
	  ...
	  $zakname=mysql_result($zk_r,0,'user');
  ...
  else
  {
	...
	if($us_n>0)
	{
	  ...
	  if($resp==''){$resp=mysql_result($us_r,0,'login');}
	}
  }
  $bgc='#FFFFFF';
  ...
  if($mfrom==1){$bgc='#CCFFFF'; $picdirect='../images/arrows/arrow_sm_up_blue.gif'; } ?>
	<tr bgcolor=<? echo($bgc);?>>
    <td><? echo($mdata);?></td>
	<td align='center'><? echo($mtimer);?></td>
    <td align='center'><img src=<? echo($picdirect);?> align='absmiddle' border=1 hSpace=4></td><td align='center'><? echo($stat);?></td>
	<td nowrap><? if ($stat=='Автор') {?><a href="personal_data.php?unum=<? echo($unum);?>" target="_blank" title="Просмотр данных автора (открывается в новом окне)" style="font-weight:bold;"><? } echo($resp); if ($stat=='Автор') {?></a><? }?></td>
    <td><a href='view_mess.php?mnum=<? echo($mnum);?>&status=<? echo($mstatus);?>'><?
  ...
  ?></a></td>
  <td><? echo($str_status);?></td>
  <td align=center><input type="checkbox" class='checkbox' name="mess_sel[<? echo($i);?>]" value="<? echo($mnum);?>"><script language='JavaScript'>arCheckboxes[<? echo($i);?>]='mess_sel[<? echo($i);?>]'</script>
  </td>
</tr><? }?>
</table>
