...
if($zpay_tip=='bank'){$pay_tip='банковский перевод';}
if($zpay_tip=='cards'){$pay_tip='карточки предоплаты WebMoney или Яндекс.деньги';}
if($zpay_tip=='cash'){$pay_tip='перечисление наличных ';}
if($zpay_tip=='post'){$pay_tip='почтовый перевод';}
if($zpay_tip=='telegraph'){$pay_tip='телеграфный перевод';}
if($zpay_tip=='wallet'){$pay_tip='перевод в кошелёк WebMoney или Яндекс.деньги';}
if($zpay_tip=='webmoney'){$pay_tip='обменный пункт WebMoney';}

if($fl=='send_mess')
{
  $S_ZK_SUMM=$last_summ;
  $letter="Здравствуйте!<br><br>Заказчик $zuser не хочет платить за работу id $S_ZAKAZ_NUM $our_summ рублей, а предлагает $last_summ рублей. Крайний срок доставки заказа: $Day/$Month/$Year.<hr>Сообщение заказчика автору:<br>$mess.";
  //echo("$letter<br>");
  send_intro_mess(0, 1, $zemail, "Проблемный заказ id $S_ZAKAZ_NUM", $letter, $S_ZAKAZ_NUM);
  inmail($admin_mail, "Проблемный заказ id $S_ZAKAZ_NUM",$letter, "From: $zemail".chr(13).chr(10).'Reply-To: '.$zemail.chr(13).chr(10).'X-Mailer: PHP/'. phpversion().chr(13).chr(10).'Content-Type: text/html; charset= windows-1251', 'O_TAX');
  ...
  //Надо выкинуть из шедулинга напоминание
...
<html>
<head>
<title>Предложить автору снизить цену</title>
...
<form action="other_tax.php" method="post" name="form_tax" onSubmit="if (isNaN(this.last_summ.value)) {alert ('Поле для стоимости работы не должно содержать ничего, кроме цифр!'); this.last_summ.focus(); this.last_summ.style.backgroundColor='yellow'; return false} else return true;"><!-- #BeginLibraryItem "/Library/block_nav_top.lbi" --><a href="../../index.php"><img src="../../images/referatsinfo2.gif" 
  ...
  <table width="100%" border="0" cellspacing="0" cellpadding="10">
        <tr>
          ...
          <!-- #EndLibraryItem -->
		    ...
		    <h5><span class="style1"><?php echo($zuser);?></span>, пожалуйста, укажите
		      сумму, которую вы готовы заплатить за эту работу:
                <input name="last_summ" type="text" value="<?php echo($our_summ);?>" size="4" onBlur="this.style.backgroundColor='';">
  р. </h5>
		    Укажите <span class="style1">крайний</span> срок получения этой работы <br>
  (<strong>Внимание!</strong> согласование цены может потребовать некоторого
  времени, обычно &#8212; 1-2 дня).:<br>            <?php
$bDay=date('d')+3;
$bMonth=date('m');
