...
*/
	...
	Function Tokenize($string,$separator="")
	{
		...
		{
$GLOBALS['cto']->nowCheck(60, __LINE__);
...
function encSubj($s) {
   ...
   // Ïðîâåðÿåì, îòâå÷àåò ëè ñòðîêà ôîðìàòó êîäèðîâàííîãî çàãîëîâêà? 
   // Åñëè äà, òî *ðàçáèðàåì* ñòðîêó, çàïîëíÿÿ ìàññèâ $val:
   //  $val[0] - èñõîäíàÿ ñòðîêà
   //  $val[1] - êîäèðîâêà
   //  $val[2] - ñïîñîá êîäèðîâàíèÿ
   //  $val[3] - êîäèðîâàííàÿ ñòðîêà
  
   // Åñëè äà, âûÿñíÿåì ñïîñîá êîäèðîâàíèÿ ïî çíà÷åíèþ $val[2] è *äåêîäèðóåì*
     if($val[2] == 'B') {
     // äåêîäèðóåì Base64
       $val[3] = base64_decode($val[3]);
     } elseif ($val[2] == 'Q') {
     // äåêîäèðóåì QuotedPrint
       $i=0;
       $len = strlen($val[3]);
       $new='';
       while ($i<$len) {
$GLOBALS['cto']->nowCheck(60, __LINE__);
     ...
     // Ïðîâåðÿåì, êîäèðîâêó, åñëè KOI8-R, òî *ïåðåêîäèðóåì* ñòðîêó â Windows-1251.
     if (!strcasecmp($val[1], 'koi8-r')) {
       $val[3] = convert_cyr_string($val[3], "k", "w");
     }
     return $val[3];
   } else {
   // Çàãîëîâîê íå îòâå÷àåò èçâåñòíîìó íàì ôîðìàòó êîäèðîâàíèÿ - âîçâðàùàåì *áåç èçìåíåíèé*
     return $s;
   }
 }
?>