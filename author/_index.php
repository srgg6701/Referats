...
//СПИСОК
var wlist=document.getElementById('work_type');
//контент контейнера списка выбора предметов
var wlistHTML=document.getElementById('file_upload').innerHTML;
var upl,lists,j,cnt1;
//счётчики списков:
var wacount=0;
var wtcount=0;
if (isNaN(files_count)) alert('Указанное вами значение для количества файлов не является числом!');
		...
		for (i=1;i<=files_count;i++) {
			...
			upl='<div>'+wlistHTML+' <? 
			//только в разделе загрузки файлов:
			if ($_REQUEST['submenu']=="upload") $cancel_sel_name=true; //отменим name, id списка, чтоб получить его по его индексу
			$Worx->buildWorxAreasList(" style=\'background-color:#F7f7f7; width:240px;\'",false);
			
			?> <input name="file_'+cnt1+'" type="file" size="80"> <input name="wprice_'+cnt1+'" id="wprice_'+cnt1+'" type="text" size="3" title="Ваша цена работы" onkeyUp="calculatePrices(this,\'wprice_customer_'+cnt1+'\');"> <input name="wprice_customer_'+cnt1+'" id="wprice_customer_'+cnt1+'" type="text" size="3" title="Цена для заказчика" onkeyUp="calculatePrices(this,\'wprice_'+cnt1+'\');"> \n<a href="#" \nonClick="parentNode.style.display=';
			upl+="'none';parentNode.innerHTML='';return false;";
			upl+='" title="Убрать"><img src="<?=$_SESSION['SITE_ROOT']?>images/delete2.gif" align="absmiddle" hspace="4" border="0" /></a></div>';
			//дописываем контент блока загрузки:
			container.insertAdjacentHTML("beforeEnd",upl);
		} counter+=i;
		//контент списка выбора типа работы:
		lists=container.getElementsByTagName('SELECT');
		//перебираем все сгениерированные клиентом списки:
		for (i=0;i<lists.length;i++) {
			//текущий сгенерированный клиентом список:
			j=i+1;
				//i: 0 1 2 3
			//проверим список - тип_работы/предмет и назначим соответствующее имя:
			if (isInt(j/2)){
				wacount++;
				//j: 1 2 3 4
				////   *   *
				//			
				lists.item(i).name='warea';
				lists.item(i).name+='_'+wacount; //1
			}else{
				wtcount++;
				//j: 1 2 3 4
				//// *   *
				lists.item(i).name='wtype';  
				lists.item(i).name+='_'+wtcount; 
			}
			
			//alert(lists.item(i).name);
			
			//выделяем в текущем списке тип работы, выбранный автором для загрузки:
			if (!counter1) lists.item(i).options[wlist.selectedIndex].selected=true;
			else if (i>counter1&&lists.item(i).name.indexOf('wprice')==-1) {
				lists.item(i).options[wlist.selectedIndex].selected=true;
			}
		} 
...
$regUser->remPassIframe(); 
  ...
  <tr height="100%">
    <td colspan="2" valign="top" style="border-top:solid 1px #CCC;"><div style="overflow:auto; height:100%;"<? 
