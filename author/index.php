...
//
var upl;
//var vl; для отладки, если нужно вывести алертом список типов работ
//счётчики списков:
//var wacount=0;
//var wtcount=0;
var tblStart="\n<table>\n  <tr>\n    <td>\n";
...
if (isNaN(files_count)) alert('Указанное вами значение для количества файлов не является числом!');
		...
		while (i<files_count) {
			...
			upl=' \n<!--СПИСОК ПРЕДМЕТОВ:-->\n'+tblStart+'<select name="work_type'+counter+'" id="work_area'+counter+'">\n';
			upl+='<? 
			
			$nl="\\n";
			
			$Worx->buildWorxTypesListOptions($sel_type);?>';
			upl+='</select>'+tblNextCell;
			
			//alert(upl);
			//СПИСОК предметов:
			upl+=' \n<!--СПИСОК ТИПОВ РАБОТ:-->\n<select style="background-color:#F7f7f7;" name="work_area'+counter+'" id="work_type'+counter+'">';
			upl+='<? 
			
			$Worx->buildWorxAreasListOptions($sel_area);?>';
			...
			var selTypes=document.getElementById('work_type'+counter);
			
			var inOptions='';
			
			for(j=0;j<selTypes.length;j++) {
				
				inOptions+='\n'+selTypes.options[j].value+' | | '+selTypeValue;
				//if (selTypes.options[j].value=="Диплом MBA") alert('MBA!');
				if (selTypes.options[j].value==selTypeValue) {
					//alert('BINGO!');
					selTypes.options[j].selected=true;
				} //vl+=selTypes.options[j].value+'\n'; //отладочная...
			}
			//alert(selTypeValue+'\n'+vl);
			i++;
			counter++; //2
